...
const fromFalsy = (x) => (x ? Right(x) : Left())
...
CombineResult.empty = () => CombineResult(Result('', '', null, false))
...
const validateElement = (commentsByClass, $element) =>
  extractClassNames($element).map((className) =>
    fromNullable(commentsByClass.get(className))
      .map(
        (comments) =>
          comments
            .map((c) => validate(className, c, $element))
            .foldMap(CombineResult, CombineResult.empty()).x
      )
      .fold(
...
const getChildren = ($element) => I.List($element.children().get())

const validateByClassName = (commentsByClass, $element, $) =>
  validateElement(commentsByClass, $element).concat(
    getChildren($element).flatMap((e) =>
    ...
    .flatMap((comments) => {
      ...
      return I.List(elements.get())
        ...
        .map(
          ($el) =>
            comments.foldMap(
              (c) => CombineResult(validate(selector, c, $el)),
              CombineResult.empty()
            ).x
        )
    })
...
module.exports = (comments, $, rootNode) => {
  ...
  const commentsByFancy = commentsBySelector.filter(
    (v, k) => !/^\.slds-/.test(k)
  )
  ...
  return validateByClassName(commentsByClass, $rootNode, $)
    ...
    .map((result) =>
      result.set('elementString', print($, result.get('element')))
    )
    .map((result) =>
      result.update('element', (e) => (e.extract ? e.extract() : e))
    )
}
